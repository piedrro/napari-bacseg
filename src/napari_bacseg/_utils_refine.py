...
def refine_mask(self, image, mask, mask_id):
    ...
    for i in range(refine_iterations):
        ...
        for j in range(len(wall_cnt)):
            ...
            if refine_mode == 0:
                box_index = np.array(
                    np.unravel_index(box_values.argmax(), box_values.shape)
                )
            else:
                box_index = np.array(
                    np.unravel_index(box_values.argmin(), box_values.shape)
                )
        ...
        wall_contour = (
            np.array(wall_contour).reshape((-1, 1, 2)).astype(np.int32)
        )
    ...
    cv2.drawContours(
        wall_mask, [wall_contour], contourIdx=-1, color=(1, 1, 1), thickness=-1
    )
