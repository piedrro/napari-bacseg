...
from qtpy.QtCore import QObject, QRunnable, QThreadPool, pyqtSignal, pyqtSlot
...
class Worker(QRunnable):
    ...
    :param callback: The function callback to run on this worker thread.
     Supplied args andkwargs will be passed through to the runner.
...
class BacSeg(QWidget):
    ...
    def __init__(self, viewer: napari.Viewer):
        ...
        self._initialise_cellpose_model = partial(
            _initialise_cellpose_model, self
        )
        self._select_custom_cellpose_model = partial(
            _select_custom_cellpose_model, self
        )
        self._select_cellpose_save_directory = partial(
            _select_cellpose_save_directory, self
        )
        self._select_cellpose_save_path = partial(
            _select_cellpose_save_path, self
        )
        ...
        self._compute_simple_cell_stats = partial(
            _compute_simple_cell_stats, self
        )
        ...
        self.download_sort_order = self.findChild(
            QComboBox, "download_sort_order"
        )
        ...
        # modify tab events
        self.modify_panzoom.clicked.connect(
            partial(self._modifyMode, "panzoom")
        )
        self.modify_segment.clicked.connect(
            partial(self._modifyMode, "segment")
        )
        self.modify_classify.clicked.connect(
            partial(self._modifyMode, "classify")
        )
        ...
        self.classify_single.clicked.connect(
            partial(self._modifyMode, "single")
        )
        self.classify_dividing.clicked.connect(
            partial(self._modifyMode, "dividing")
        )
        self.classify_divided.clicked.connect(
            partial(self._modifyMode, "divided")
        )
        self.classify_vertical.clicked.connect(
            partial(self._modifyMode, "vertical")
        )
        self.classify_broken.clicked.connect(
            partial(self._modifyMode, "broken")
        )
        self.classify_edge.clicked.connect(partial(self._modifyMode, "edge"))
        self.modify_viewmasks.stateChanged.connect(
            partial(self._viewerControls, "viewmasks")
        )
        self.modify_viewlabels.stateChanged.connect(
            partial(self._viewerControls, "viewlabels")
        )
        ...
        self.modify_deleteallmasks.clicked.connect(
            partial(self._deleteallmasks, mode="all")
        )
        self.modify_deleteactivemasks.clicked.connect(
            partial(self._deleteallmasks, mode="active")
        )
        self.modify_deleteactiveimage.clicked.connect(
            partial(self._delete_active_image, mode="active")
        )
        self.modify_deleteotherimages.clicked.connect(
            partial(self._delete_active_image, mode="other")
        )
        self.find_next.clicked.connect(partial(self._sort_cells, "next"))
        self.find_previous.clicked.connect(
            partial(self._sort_cells, "previous")
        )
        ...
        self.export_statistics_active.clicked.connect(
            partial(self._export_statistics, "active")
        )
        self.export_statistics_all.clicked.connect(
            partial(self._export_statistics, "all")
        )

        # oufti events
        self.oufti_generate_all_midlines.clicked.connect(
            partial(self.generate_midlines, mode="all")
        )
        self.oufti_generate_active_midlines.clicked.connect(
            partial(self.generate_midlines, mode="active")
        )
        ...
        self.oufti_centre_all_midlines.clicked.connect(
            partial(self.centre_oufti_midlines, mode="all")
        )
        self.oufti_centre_active_midlines.clicked.connect(
            partial(self.centre_oufti_midlines, mode="active")
        )

        # upload tab events
        self.upload_all.clicked.connect(
            partial(self._uploadDatabase, mode="all")
        )
        self.upload_active.clicked.connect(
            partial(self._uploadDatabase, mode="active")
        )
        ...
        self.viewer.bind_key(
            key="t", func=partial(self._modifyMode, "toggle"), overwrite=True
        )
        self.viewer.bind_key(
            key="a", func=partial(self._modifyMode, "add"), overwrite=True
        )
        self.viewer.bind_key(
            key="e", func=partial(self._modifyMode, "extend"), overwrite=True
        )
        self.viewer.bind_key(
            key="j", func=partial(self._modifyMode, "join"), overwrite=True
        )
        self.viewer.bind_key(
            key="s", func=partial(self._modifyMode, "split"), overwrite=True
        )
        self.viewer.bind_key(
            key="d", func=partial(self._modifyMode, "delete"), overwrite=True
        )
        self.viewer.bind_key(
            key="r", func=partial(self._modifyMode, "refine"), overwrite=True
        )

        self.viewer.bind_key(
            key="Control-1",
            func=partial(self._modifyMode, "single"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-2",
            func=partial(self._modifyMode, "dividing"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-3",
            func=partial(self._modifyMode, "divided"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-4",
            func=partial(self._modifyMode, "vertical"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-5",
            func=partial(self._modifyMode, "broken"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-6",
            func=partial(self._modifyMode, "edge"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="F1", func=partial(self._modifyMode, "panzoom"), overwrite=True
        )
        self.viewer.bind_key(
            key="F2", func=partial(self._modifyMode, "segment"), overwrite=True
        )
        self.viewer.bind_key(
            key="F3",
            func=partial(self._modifyMode, "classify"),
            overwrite=True,
        )

        self.viewer.bind_key(
            key="h", func=partial(self._viewerControls, "h"), overwrite=True
        )
        self.viewer.bind_key(
            key="i", func=partial(self._viewerControls, "i"), overwrite=True
        )
        self.viewer.bind_key(
            key="o", func=partial(self._viewerControls, "o"), overwrite=True
        )
        self.viewer.bind_key(
            key="x", func=partial(self._viewerControls, "x"), overwrite=True
        )
        self.viewer.bind_key(
            key="z", func=partial(self._viewerControls, "z"), overwrite=True
        )
        self.viewer.bind_key(
            key="c", func=partial(self._viewerControls, "c"), overwrite=True
        )
        self.viewer.bind_key(
            key="Right",
            func=partial(self._imageControls, "Right"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Left",
            func=partial(self._imageControls, "Left"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="u",
            func=partial(self._imageControls, "Upload"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-d",
            func=partial(self._deleteallmasks, mode="active"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-Shift-d",
            func=partial(self._deleteallmasks, mode="all"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-i",
            func=partial(self._delete_active_image, mode="active"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-Shift-i",
            func=partial(self._delete_active_image, mode="other"),
            overwrite=True,
        )
        ...
        self.viewer.bind_key(
            key="Control-u",
            func=partial(self._uploadDatabase, mode="active"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-Shift-u",
            func=partial(self._uploadDatabase, mode="all"),
            overwrite=True,
        )

        self.viewer.bind_key(
            key="Control-Left",
            func=partial(self._manual_align_channels, "left", mode="active"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-Right",
            func=partial(self._manual_align_channels, "right", mode="active"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-Up",
            func=partial(self._manual_align_channels, "up", mode="active"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Control-Down",
            func=partial(self._manual_align_channels, "down", mode="active"),
            overwrite=True,
        )

        self.viewer.bind_key(
            key="Alt-Left",
            func=partial(self._manual_align_channels, "left", mode="all"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Alt-Right",
            func=partial(self._manual_align_channels, "right", mode="all"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Alt-Up",
            func=partial(self._manual_align_channels, "up", mode="all"),
            overwrite=True,
        )
        self.viewer.bind_key(
            key="Alt-Down",
            func=partial(self._manual_align_channels, "down", mode="all"),
            overwrite=True,
        )
    ...
    def _export_statistics(self, mode="active"):
        ...
        if os.path.isdir(colicoords_dir) is not True:
        ...
        if os.path.isdir(path):
            ...
            self.process_cell_statistics = partial(
                process_cell_statistics, self
            )
            ...
            worker.signals.progress.connect(
                partial(self._bacseg_progresbar, progressbar="export")
            )
    ...
    def _refine_bacseg(self):
        ...
        worker.signals.progress.connect(
            partial(self._bacseg_progresbar, progressbar="modify")
        )
    ...
    def _uploadDatabase(self, viewer=None, mode=""):
        try:
            if (
                self.database_path != ""
                and os.path.exists(self.database_path) is True
            ):
                if self.unfolded is True:
                    self.fold_images()
                ...
                self._upload_bacseg_database = partial(
                    _upload_bacseg_database, self
                )

                worker = Worker(self._upload_bacseg_database, mode=mode)
                worker.signals.progress.connect(
                    partial(self._bacseg_progresbar, progressbar="database")
                )
                self.threadpool.start(worker)
    ...
    def _downloadDatabase(self, viewer=None):
        try:
            if (
                ...
                self.get_filtered_database_metadata = partial(
                    get_filtered_database_metadata, self
                )
                ...
                if len(file_paths) == 0:
                    ...
                    worker.signals.progress.connect(
                        partial(
                            self._bacseg_progresbar, progressbar="database"
                        )
                    )
                    self.threadpool.start(worker)
    ...
    def _bacseg_progresbar(self, progress, progressbar):
    ...
    def _importDialog(self):
        ...
        if "" in paths or paths is []:
            show_info("No file/folder selected")

        else:
            if import_mode == "Images":
                self.import_images = partial(
                    napari_bacseg._utils.import_images, self
                )

                worker = Worker(self.import_images, file_paths=paths)
                worker.signals.result.connect(self._process_import)
                worker.signals.progress.connect(
                    partial(self._bacseg_progresbar, progressbar="import")
                )
                self.threadpool.start(worker)

            if import_mode == "NanoImager Data":
                self.read_nim_directory = partial(
                    napari_bacseg._utils.read_nim_directory, self
                )
                self.read_nim_images = partial(
                    napari_bacseg._utils.read_nim_images, self
                )
            ...
            if import_mode == "Mask (.tif) Segmentation(s)":
                self.import_masks = partial(
                    napari_bacseg._utils.import_masks, self
                )
            ...
            if import_mode == "ImageJ files(s)":
                self.import_imagej = partial(
                    napari_bacseg._utils.import_imagej, self
                )
    ...
    def _export(self, mode):
        ...
        if execute_export is True:
            self.export_files = partial(
                napari_bacseg._utils.export_files, self
            )
    ...
    def _trainCellpose(self):
        ...
        worker.signals.progress.connect(
            partial(self._bacseg_progresbar, progressbar="cellpose_train")
        )
        self.threadpool.start(worker)
    ...
    def _segmentActive(self):
        ...
        worker.signals.progress.connect(
            partial(self._bacseg_progresbar, progressbar="cellpose")
        )
        self.threadpool.start(worker)
    ...
    def _process_import(self, imported_data, rearrange=True):
        ...
        if self.clear_previous.isChecked() is True:
        ...
        for layer_name, layer_data in imported_images.items():
            ...
            if (
                self.clear_previous.isChecked() is False
        ...
        if "532" in layer_names and rearrange is True:
            layer_name = "532"
        ...
        if "Cy3" in layer_names and rearrange is True:
            layer_name = "Cy3"
            num_layers = len(self.viewer.layers)
            layer_ref = self.viewer.layers[layer_name]
            layer_index = self.viewer.layers.index(layer_name)
    ...
    def _autoClassify(self, reset=False):
        ...
        for i in range(len(mask_stack)):
            ...
            if len(label_ids) == 1 or reset is True:
                ...
                for mask_id in mask_ids:
                    if mask_id != 0:
                        ...
                        # appends contour to list if the bounding coordinates\
                        # are along the edge of the image
                        if (
                            y1 > 0
                            ...
                            label[mask is mask_id] = 1

                        else:
                            label[mask is mask_id] = 6
