...
class BacSeg(QWidget):
    ...
    def __init__(self, viewer: napari.Viewer):
        ...
        self.picasso_image_channel = self.findChild(
            QComboBox, "picasso_image_channel"
        )
        self.picasso_box_size = self.findChild(QComboBox, "picasso_box_size")
        self.picasso_min_net_gradient = self.findChild(
            QLineEdit, "picasso_min_net_gradient"
        )
        ...
        self.picasso_vis_opacity = self.findChild(
            QComboBox, "picasso_vis_opacity"
        )
        self.picasso_vis_edge_width = self.findChild(
            QComboBox, "picasso_vis_edge_width"
        )
        self.picasso_show_vis = self.findChild(QCheckBox, "picasso_show_vis")
        self.picasso_image_frames = self.findChild(
            QComboBox, "picasso_image_frames"
        )
        self.picasso_progressbar = self.findChild(
            QProgressBar, "picasso_progressbar"
        )
        self.picasso_export = self.findChild(QPushButton, "picasso_export")
        self.picasso_export_mode = self.findChild(
            QComboBox, "picasso_export_mode"
        )
        self.picasso_filter_localisations = self.findChild(
            QCheckBox, "picasso_filter_localisations"
        )
        ...
        self.cellpose_auto_classify = self.findChild(
            QCheckBox, "cellpose_auto_classify"
        )
        ...
        self.picasso_vis_size.currentIndexChanged.connect(
            self.update_localisation_visualisation
        )
        self.picasso_vis_mode.currentIndexChanged.connect(
            self.update_localisation_visualisation
        )
        self.picasso_vis_opacity.currentIndexChanged.connect(
            self.update_localisation_visualisation
        )
        self.picasso_vis_edge_width.currentIndexChanged.connect(
            self.update_localisation_visualisation
        )
    ...
    def export_picasso_localisations(self, event=None):
        try:
            if (
                hasattr(self, "detected_locs") == False
                and hasattr(self, "fitted_locs") == False
            ):
                show_info(
                    "No localisations found, please detect + fit localisations first"
                )
            elif (
                hasattr(self, "detected_locs") == True
                and hasattr(self, "fitted_locs") == False
            ):
                show_info(
                    "No fitted localisations found, please fit localisations first"
                )
            elif (
                hasattr(self, "detected_locs") == True
                and hasattr(self, "fitted_locs") == True
            ):
                desktop = os.path.expanduser("~/Desktop")
                export_directory = QFileDialog.getExistingDirectory(
                ...
                if os.path.isdir(export_directory):
                    ...
                    if image_channel in image_layers:
                        image_name = self.viewer.layers[
                            image_channel
                        ].metadata[0]["image_name"]
                        image_path = self.viewer.layers[
                            image_channel
                        ...
                        localisation_data = self.get_localisation_data(
                            self.fitted_locs
                        )
                        ...
                        if export_mode == ".csv":
                            export_path = os.path.join(
                                export_directory,
                                image_name + "_picasso_localisations.csv",
                            )
                            ...
                            show_info(
                                "Picasso localisations exported to: "
                                + export_path
                            )
                            ...
                            export_path = os.path.join(
                                export_directory,
                                image_name + "_picasso_localisations.pos.out",
                            )
                            export_path = os.path.abspath(export_path)

                            st_locs = localisation_data[
                                [
                                    "frame",
                                    "x",
                                    "y",
                            ...
                            hdf5_path = os.path.join(
                                export_directory,
                                image_name + "_picasso_localisations.hdf5",
                            )
                            yaml_path = os.path.join(
                                export_directory,
                                image_name + "_picasso_localisations.yaml",
                            )
                            ...
                            structured_arr = localisation_data.to_records(
                                index=False
                            )
                            ...
                            with h5py.File(hdf5_path, "w") as hdf_file:
                                ...
                                dataset = hdf_file.create_dataset(
                                    "locs", data=structured_arr
                                )

                            picasso_channel = (
                                self.picasso_image_channel.currentText()
                            )
                            image_shape = self.viewer.layers[
                                picasso_channel
                            ].data.shape
                            ...
                            with open(yaml_path, "w") as file:
                                ...
                                file.write("---\n")  # Document separator
                                yaml.dump(
                                    data2, file, default_flow_style=False
                                )
    ...
    def _fit_localisations_cleanup(self):
        try:
            ...
            self.localisation_centres = self.get_localisation_centres(
                self.fitted_locs
            )
            ...
            num_frames = len(
                np.unique([loc[0] for loc in self.localisation_centres])
            )
            num_locs = len(self.detected_locs)

            show_info(
                f"Picasso Fitted {num_locs} localisations in {num_frames} frame(s)"
            )

        except:
            pass

    def _fit_localisations(
        self,
        ...
        try:
            ...
            if n_detected_frames != image_data.shape[0]:
                show_info(
                    "Picasso can only Detect AND Fit localisations with same image frame mode"
                )
            else:
                detected_loc_spots = localize.get_spots(
                    image_data, self.detected_locs, box_size, camera_info
                )

                show_info(
                    f"Picasso fitting {len(self.detected_locs)} spots..."
                )
                ...
                if method == "lq":
                    if use_gpufit:
                        ...
                        em = gain > 1
                        self.fitted_locs = gausslq.locs_from_fits_gpufit(
                            self.detected_locs, theta, box_size, em
                        )
                    else:
                        fs = gausslq.fit_spots_parallel(
                            detected_loc_spots, asynch=True
                        )
                        ...
                        em = gain > 1
                        self.fitted_locs = gausslq.locs_from_fits(
                            self.detected_locs, theta, box_size, em
                        )
                ...
                if self.picasso_filter_localisations.isChecked():
                    self.fitted_locs = self.filter_localisations(
                        self.fitted_locs
                    )
    ...
    def _detect_localisations(
        ...
        try:
            ...
            curr, futures = localize.identify_async(
                image_data, min_net_gradient, box_size, roi=None
            )
            ...
            if self.picasso_filter_localisations.isChecked():
                self.detected_locs = self.filter_localisations(
                    self.detected_locs
                )
    ...
    def filter_localisations(self, locs):
        ...
        try:
            ...
            if n_removed > 0:
                show_info(
                    f"Picasso removed {n_removed} localisations outside of segmentations"
                )
    ...
    def _detect_localisations_cleanup(self):
        try:
            ...
            show_info(
                f"Picasso Detected {num_locs} localisations in {num_frames} frame(s)"
            )
    ...
    def display_localisations(self):
        if (
            hasattr(self, "localisation_centres")
            and self.picasso_show_vis.isChecked()
        ):
            try:
                ...
                vis_edge_width = float(
                    self.picasso_vis_edge_width.currentText()
                )
        ...
        try:
            ...
            if min_net_gradient.isdigit() and image_channel != "":
                ...
                worker.signals.finished.connect(
                    self._detect_localisations_cleanup
                )
    ...
    def fit_picasso_localisations(self):
        if hasattr(self, "localisation_centres") == False:
            show_info(
                "No localisations detected, please detect localisations first"
            )
        else:
            image_channel = self.picasso_image_channel.currentText()
            box_size = int(self.picasso_box_size.currentText())
            ...
            if min_net_gradient.isdigit() and image_channel != "":
                ...
                worker.signals.progress.connect(
                    partial(self._Progresbar, progressbar="picasso")
                )
                worker.signals.finished.connect(
                    self._fit_localisations_cleanup
                )
                self.threadpool.start(worker)
    ...
    def _updateSegmentationCombo(self):
        ...
        export_layers.extend(
            [
                "All Channels (Stack)",
                "All Channels (Horizontal Stack)",
                "All Channels (Vertical Stack)",
                "First Three Channels (RGB)",
            ]
        )
