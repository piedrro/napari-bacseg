...
def _process_cellpose(self, segmentation_data):
    if len(segmentation_data) > 0:
        ...
        if self.segLayer.data.shape is not masks.shape:
        ...
        if self.cellpose_resetimage.isChecked() is True:
...
def _initialise_cellpose_model(
    ...
    if model_type == "custom":
        if model_path not in ["", None] and os.path.isfile(model_path) is True:
            ...
            if "omnipose_" in model_name and mode == "eval":
                ...
                show_info("BacSeg Cannot train Omnipose models")
                load_model = False
    ...
    try:
        ...
        if model is not None:
            ...
            with warnings.catch_warnings():
                ...
                show_info(
                    f"Training Complete. Cellpose model path: \
                    {pretrained_model}"
                )
