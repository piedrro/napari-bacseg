        ...
        try:
            ...
            for j in range(len(channels)):
                ...
                if channel in measurement_channels:
                    dat = measurement[measurement["channel"] is channel]
        ...
        try:
            for j in range(len(layer_names)):
                ...
                if meta["image_name"] != "missing image channel":
                    ...
                    if self.cellpose_segmentation is True:
                        meta[
...
def upload_AKSEG_files(path):
    ...
    try:
        ...
        for j, layer in enumerate(channel_list):
            ...
            if file_name in metadata_file_names:
                try:
                    date_uploaded = user_metadata[
                        (user_metadata["file_name"] is file_name)
                        & (user_metadata["folder"] is folder)
                    ]["date_uploaded"].item()
                except Exception:
            ...
            # stops user from overwriting BacSeg files,
            # unless they have opened them from BacSeg for curation
            if (
                akseg_hash in metadata_akseg_hash
                and import_mode != "BacSeg"
                and overwrite_images is False
                and overwrite_masks is False
                and overwrite_metadata is False
            ):
                show_info(
                    "file already exists  in BacSeg Database:   " + file_name
                )
                file_metadata = None
                ...
                if import_mode == "BacSeg":
                    if overwrite_selected_metadata is True:
                        show_info(
                            "Overwriting selected metadata on BacSeg\
                             Database:   "
                            + file_name
                        )
                        ...
                        show_info(
                            "Editing file on BacSeg Database:   " + file_name
                        )
                    ...
                    show_info(
                        "Overwriting image on BacSeg Database:   " + file_name
                    )
...
def generate_upload_tempfiles(
    ...
    if os.path.isdir(upload_dir) is not True:
        os.mkdir(upload_dir)
    else:
    ...
    try:
        ...
        if os.path.exists(database_path) is False:
            ...
            if (
                ...
                show_info(
                    "Please fill out channel (all channels) and image \
                    metadata before uploading files"
                )
                ...
                if segChannel == "":
                    ...
                    if mode == "active":
                        results = upload_AKSEG_files(upload_tempfiles[0])
                        ...
                        with Pool(4) as pool:
                            ...
                            results = [
                                dat for dat in results if results is not None
                            ]
...
def get_filtered_database_metadata(self):
    ...
    if os.path.isfile(user_metadata_path) is False:
        ...
        for key, value in database_metadata.items():
            user_metadata = user_metadata[user_metadata[key] is value]

        if self.upload_segmentation_combo.currentIndex() == 1:
            user_metadata = user_metadata[user_metadata["segmented"] is False]
        if self.upload_segmentation_combo.currentIndex() == 2:
            user_metadata = user_metadata[user_metadata["segmented"] is True]
        if self.upload_segmentation_combo.currentIndex() == 3:
            user_metadata = user_metadata[
                (user_metadata["segmented"] is True)
                & (user_metadata["segmentation_curated"] is True)
            ]
        if self.upload_label_combo.currentIndex() == 1:
            user_metadata = user_metadata[user_metadata["labelled"] is False]
        if self.upload_label_combo.currentIndex() == 2:
            user_metadata = user_metadata[user_metadata["labelled"] is False]
        if self.upload_label_combo.currentIndex() == 3:
            user_metadata = user_metadata[
                (user_metadata["labelled"] is False)
                & (user_metadata["label_curated"] is True)
            ]
