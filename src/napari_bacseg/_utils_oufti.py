            ...
            if "midlines" in meta.keys():
                ...
                for j in range(len(mid_lines)):
                    ...
                    (
                        ...
                        _,
                        _,
                    ) = get_midline_boundary_lines(mid_line, cnt, smooth=False)

                    mid_line = centre_midline(
                        left_line,
                        right_line,
                        mid_line,
                        export_segments=len(mid_line),
                    )

                    mid_line = interpolate_data(
                        mid_line, export_segments=len(mid_line)
                    )
...
def midline_edit_toggle(self, viewer=None):
    ...
    if "center_lines" in layer_names:
        ...
        if self.shapeLayer.mode == "pan_zoom":
            ...
            self.shapeLayer.selected_data = set(
                np.arange(len(self.shapeLayer.data))
            )
...
def generate_midlines(self, mode="all"):
    ...
    if "center_lines" not in layer_names:
        self.shapeLayer = self.viewer.add_shapes(
            shape_type="path",
            edge_width=0.5,
            opacity=0.5,
            edge_color="red",
            ...
            if id != 0:
                try:
                    ...
                    if h > w:
                        ...
                        mid_line = get_voronoi_midline(
                            cnt90, extend=False, vertices=vertexes
                        )

                        if mid_line is not None:
                            mid_line = rotate_model(
                                mid_line, shift_xy, angle=-90
                            )
                    else:
                        mid_line = get_voronoi_midline(
                            cnt, extend=False, vertices=vertexes
                        )

                    if mid_line is not None:
                        mid_line = interpolate_data(
                            mid_line, export_segments=vertexes
                        )
...
def get_voronoi_midline(
    ...
    try:
        ...
        mid_line = fit_polyline(
            verts[:, 0],
            verts[:, 1],
            polys=[2, 3],
            margin=poly_margin,
            poly_limits=poly_limits,
        )
...
def fit_polyline(x, y, polys=[2, 3], margin=50, poly_limits=None):
    with warnings.catch_warnings():
        ...
        for poly in polys:
            params, residuals, rank, singular_values, rcond = np.polyfit(
                x.copy(), y.copy(), poly, full=True
            )

            residual_list.append(residuals[0])
            param_list.append(params)

        residual_list, param_list = zip(
            *sorted(zip(residual_list, param_list), reverse=True)
        )
        ...
        if poly_limits is not None:
            if len(poly_limits) is len(poly_params):
...
def get_mesh(oufti_dict, bisector_length=100, n_segments=100):
    ...
    mid_line_segments = [
        LineString(
            [
                midline_lineString.interpolate(distance - 0.01),
                midline_lineString.interpolate(distance + 0.01),
            ]
        )
    ...
    for segment in mid_line_segments:
        ...
        if (
            left_intersection.type == "Point"
            and right_intersection.type == "Point"
        ):
            right_line_data.append(
                np.array(left_intersection.xy).reshape(2).tolist()
            )
            left_line_data.append(
                np.array(right_intersection.xy).reshape(2).tolist()
            )
    ...
    try:
        ...
        for i, poly in enumerate(polygons):
            inside_points = [
                ...
                if poly.contains(point) is True
            ]
        ...
        if len(intersection_lengths) != 0:
            intersection_lengths, intersection_ids = zip(
                *sorted(
                    zip(intersection_lengths, intersection_ids), reverse=True
                )
            )
...
def check_midlines_intersecting(cnt, midline):
    ...
    if cnt_LinearRing.intersection(midline_start_point).length == 0:
        start_point = np.array(
            nearest_points(cnt_LinearRing, midline_start_point)[0].xy
        ).T
    ...
    if cnt_LinearRing.intersection(midline_end_point).length == 0:
        end_point = np.array(
            nearest_points(cnt_LinearRing, midline_end_point)[-1].xy
        ).T
...
def get_midline_boundary_lines(midline, cnt, smooth=True, n_segments=100):
    ...
    if cnt_LinearRing.intersection(midline_start_point).length == 0:
        start_intersection = np.array(
            nearest_points(cnt_LinearRing, midline_start_point)[-1].xy
        ).T[0]

    if cnt_LinearRing.intersection(midline_end_point).length == 0:
        end_intersection = np.array(
            nearest_points(cnt_LinearRing, midline_end_point)[0].xy
        ).T[0]
    ...
    end_indicies, end_intersections = zip(
        *sorted(zip(end_indicies, end_intersections), reverse=False)
    )
...
def remove_intersecting(line, intersecting_line):
    ...
    intersection = np.array(
        [[geom.xy[0][0], geom.xy[1][0]] for geom in intersection.geoms]
    )
...
def snap_midline_to_contour(cnt, midline, fit=False):
    try:
        ...
        if fit is True:
            midline = fit_polyline(
                midline[:, 0],
                midline[:, 1],
                polys=[2],
                margin=5,
                poly_limits=[None, None],
            )

            del_indexes = []

            for i, point in enumerate(midline):
                if cnt_poly.contains(Point(point)) is False:
    ...
    if midlines is None:
        for i in range(len(contours)):
            try:
                ...
                left_line, right_line, _, cnt_array = get_boundary_lines(
                    mid_line, cnt, smooth=True, n_segments=100
                )

                mid_line, end_intersections = trim_midline(
                    left_line, right_line, mid_line, margin=10
                )
        ...
        for i in range(len(midlines)):
            try:
                ...
                if index is not None:
                    ...
                    if mesh_dilation == 0:
                        midline = snap_midline_to_contour(
                            cnt, midline, fit=False
                        )
                    else:
                        cnt = dilate_contour(cnt, mesh_dilation)
                        midline = snap_midline_to_contour(
                            cnt, midline, fit=True
                        )

                    left_line, right_line, _, cnt_array = get_boundary_lines(
                        midline, cnt, smooth=True, n_segments=100
                    )

                    _, end_intersections = trim_midline(
                        left_line, right_line, midline, margin=10
                    )
...
def centre_midline(
    left_line, right_line, midline, fit_segments=100, export_segments=6
):
    left_LineString = LineString(left_line)
    right_LineString = LineString(right_line)
    ...
    for i in range(1, len(midline) - 1):
        ...
        left_intersection = np.array(
            nearest_points(left_LineString, point)[0].xy
        ).T[0]
        right_intersection = np.array(
            nearest_points(right_LineString, point)[0].xy
        ).T[0]
...
def interpolate_data(line, export_segments=100):
    ...
    interpolator = interpolate.interp1d(
        distance, line, kind="quadratic", axis=0
    )
...
def export_oufti(image, oufti_data, file_path):
    ...
    microbeTrackerParamsString = (
        "% This file contains MicrobeTracker "
        "settings optimized for wildtype E. coli"
        " cells at 0.114 um/pixel resolution"
        " (using algorithm 4)\n\nalgorithm"
    ...
    paramString = np.empty(
        (len(microbeTrackerParamsString.split("\n")), 1), dtype=object
    )
