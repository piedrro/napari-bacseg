...
def import_imagej(self, progress_callback, paths):
    ...
    if len(paths) == 1:
        ...
        if os.path.isfile(paths) is True:
            file_paths = [paths]
...
def read_nim_directory(self, path):
        ...
        try:
            ...
            if "ImageDescription" in tif_tags:
                ...
                if True in laseractive:
                    ...
                    power = laserpowers[laseractive is True].max()

                    laser_index = np.where(laserpowers is power)
    ...
    for i in range(len(positions)):
        ...
        data = files[(files["posX"] is posX) & (files["posY"] is posY)]
...
def read_nim_images(self, progress_callback, measurements, channels):
    ...
    for i in range(len(measurements)):
        ...
        for j in range(len(channels)):
            ...
            if channel in measurement_channels:
                dat = measurement[measurement["laser"] is channel]
...
def import_dataset(self, progress_callback, paths):
    ...
    if "images" in folders and "masks" in folders:
        ...
        for i in range(len(image_paths)):
            ...
            show_info(
                "loading image " + str(i + 1) + " of " + str(len(image_paths))
            )
...
def import_oufti(self, progress_callback, file_paths):
    ...
    for i in range(len(mat_files)):
        try:
            ...
            show_info(
                "loading image " + str(i + 1) + " of " + str(len(mat_files))
            )
...
def import_JSON(self, progress_callback, file_paths):
    ...
    for i in range(len(json_files)):
        try:
            ...
            show_info(
                "loading image " + str(i + 1) + " of " + str(len(json_files))
            )
    ...
    for layer in layer_names:
        ...
        for i in range(len(image_stack)):
            ...
            for index in indices:
                ...
                if mask_path is not image_path:
...
def export_files(self, progress_callback, mode):
    ...
    for i, dim in enumerate(dim_list):
        ...
        if os.path.isdir(export_path) is not True:
            show_info(
            ...
            if os.path.isfile(file_path) is True and overwrite is False:
                ...
                if self.export_mode.currentText() == "Export Oufti":
                    try:
                        ...
                        raise Exception(
                            "BacSeg can't load Cellpose and OUFTI \
                            dependencies simultaneously. \
                            Restart BacSeg, \
                            reload images/masks, then export Oufti"
                        )
                ...
                if self.export_mode.currentText() == "Export ImageJ":
                    ...
                    if mode == "rgb":
                        show_info(
                            "ImageJ can't handle RGB images with annotations,\
                             export as image stack instead..."
                        )
    ...
    try:
        if (
            ...
            if self.active_layer.metadata is {}:
                ...
                if (
                    self.classLayer.data.shape
                    is not self.active_layer.data.shape
                ):
