...
def _create_bacseg_database(self, viewer=None, database_name="BacSeg"):
    ...
    if os.path.isdir(path):
        ...
        if os.path.exists(database_directory) is True:
            show_info(
                f"{database_name} Database already exists at location\
                 {database_directory}"
            )

        else:
            show_info(
                f"Creating {database_name} Database at location\
                 {database_directory}"
            )
            ...
            for meta_item in database_metadata_list:
                txt_meta = f"# {database_name} Database Metadata: \
                {meta_item} (Add new entries below):"
...
def generate_txt_metadata(database_directory):
    ...
    if os.path.exists:
        ...
        for key, value in akmeta.items():
            txt_meta = f"# {database_name} Database Metadata: {key} \
        ...
        for user in users:
            ...
            for i in range(1, 4):
                ...
                item_list = (
                    user_metadata[(user_metadata["User Initial"] is user)][
                        f"User Meta #{i}"
                    ]
                    .dropna()
                    .tolist()
        ...
        for meta_key, meta_values in dbmeta.items():
            if meta_key in control_dict.keys():
                ...
                try:
                    ...
                    if (
                        len(combo_box_items) > 1
                        and combo_box_items is not meta_values
                    ):
                        ...
                        txt_meta_path = pathlib.PurePath(
                            database_directory,
                            "Metadata",
                            f"{database_name} Database Metadata \
                            [{meta_key}].txt",
                        )
                        ...
                        if os.path.exists(txt_meta_path):
                            ...
                            txt_meta = f"# {database_name} Database Metadata:\
                             {meta_key} (Add new entries below):"
